...
TestCaseExecutionResult TestCaseDescendant::Execute(TestCaseArgument *arg) {
    TestCaseExecutionResult res;
    int id = 0;
    DefFileReader dfr;
    MetaData *pMD = dfr.Read(arg->scriptsFolder + _MSTR(testDescendant/Defs.txt));
    ScriptReader sr;
    ...
    std::string line;
    std::string jsonline;
    std::ifstream jsonfile("../Core/TestCases/files/testDescendant/Json.txt");
    if (jsonfile.is_open()) {
