//
// Created by Erandika Ovitigala on 3/2/2023.
//
...
TestCaseExecutionResult TestGetSubTreeWithValue::Execute(TestCaseArgument *arg) {
    TestCaseExecutionResult res;
    int id = 0;
    DefFileReader dfr;
    MetaData *pMD = dfr.Read(arg->scriptsFolder + _MSTR(testGetSubTreeWithValue/Defs.txt));
    ScriptReader sr;
    ...
    std::string line;
    std::string jsonline;
    std::ifstream jsonfile("../Core/TestCases/files/testGetSubTreeWithValue/Json.txt");
    if (jsonfile.is_open()) {
