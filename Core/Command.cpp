    ...
    switch (ulCommand) {
        ...
        case COMMAND_TYPE_IS_HAVING_RIGHT_SUBSTRING: {
            if (ENTITY_TYPE_STRING == pArg->ul_Type) {
                ...
                MSTRING sArg = pStrArg->GetValue();
                pBoolRes->SetValue(
                        pString->GetValue().substr(pString->GetValue().length() - sArg.length(), sArg.length()) ==
                        sArg);
            }
            break;
        }
        ...
        case COMMAND_TYPE_GET_DIFFERENCE_BY_STRING: {
            ...
            if (pString->GetValue() != "") {
                ...
                pStrRes->SetValue(std::to_string(
                        DateTimeOperations::GetDifferenceByString(pString->ToString(), pStrArg->ToString())));
            }
            break;
        }
...
//        }
    ...
    else if(COMMAND_TYPE_GET_SUBLIST == ulCommand){
        if(pArg !=0 && pArg->ul_Type == ENTITY_TYPE_LIST){
            PENTITYLIST pArgList = (PENTITYLIST)(pArg);
            ...
            pListRes = pEntityList->GetSubList(stIndex->GetValue(),noOfElem->GetValue());
        }else{
            //Invalid Argument
            PENTITYLIST pEmptyRes;
            MemoryManager::Inst.CreateObject(&pEmptyRes);
            pListRes = pEmptyRes;
        }
    }
